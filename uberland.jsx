...
import {
  ...
  addDoc, onSnapshot, query, orderBy, serverTimestamp, updateDoc
...
// ── Constants ──────────────────────────────────────────────
const C = {
  ocean: "#0a7ea4", deep: "#064e6e", sand: "#f5d98b",
  sunset: "#f4845f", coral: "#e85d4a", palm: "#2d7a4f", dark: "#0d1b2a"
};
const GCASH = "09300291729";
const ADMIN_PASS = "uberland2024";
const SUB_PRICE = 299;
const VIOLATIONS = [
  "Harassment / Rude Behavior", "Asking Too Much Fare", "Dangerous Driving",
  "Threatening Passenger", "Sexual Harassment", "Fraud / Scam",
  "Did Not Follow Route", "Other Violation"
];
...
function Inp({ ph, val, set, type = "text", style }) {
  return (
    <input type={type} placeholder={ph} value={val} onChange={e => set(e.target.value)}
      style={{ width: "100%", padding: "11px 15px", borderRadius: 11, border: "1.5px solid rgba(255,255,255,0.15)", background: "rgba(255,255,255,0.07)", color: "#fff", fontFamily: "Nunito", fontSize: 14, outline: "none", ...style }} />
  );
}
...
function Btn({ children, onClick, style, disabled, color }) {
  return (
    <button onClick={onClick} disabled={disabled}
      style={{ background: color || `linear-gradient(135deg,${C.sunset},${C.coral})`, border: "none", borderRadius: 11, padding: "11px 22px", color: "#fff", fontFamily: "Nunito", fontWeight: 800, fontSize: 14, cursor: disabled ? "not-allowed" : "pointer", opacity: disabled ? 0.5 : 1, ...style }}>
      {children}
    </button>
  );
}
...
function Card({ children, style }) {
  return (
    <div style={{ background: "rgba(13,27,42,0.88)", backdropFilter: "blur(16px)", border: "1px solid rgba(255,255,255,0.1)", borderRadius: 18, padding: 20, ...style }}>
      {children}
    </div>
  );
}
...
function BG() {
  return (
    <div style={{ position: "fixed", inset: 0, zIndex: 0, overflow: "hidden", pointerEvents: "none" }}>
      ...
      {[0, 1, 2].map(i => (
        <div key={i} className="wave" style={{ position: "absolute", bottom: `${i * 11}%`, left: 0, right: 0, height: 36, background: `rgba(10,126,164,${0.14 + i * 0.07})`, borderRadius: "50% 50% 0 0/18px 18px 0 0", animationDelay: `${i * 0.7}s` }} />
      ))}
...
function ViolationWarning() {
  return (
    <div style={{ background: "linear-gradient(135deg,rgba(232,93,74,0.18),rgba(180,30,30,0.22))", border: "1.5px solid rgba(232,93,74,0.5)", borderRadius: 12, padding: 13, fontSize: 12, lineHeight: 1.85, color: "#ffd0c8" }}>
  ...
  async function submit() {
    ...
    try {
      await addDoc(collection(db, "reports"), {
        ...
        reportedUserVehicle: peer.vehicle || "",
        reportedUserLicense: peer.license || "",
        ...
        <div style={{ display: "flex", flexDirection: "column", gap: 7, marginBottom: 14 }}>
          {VIOLATIONS.map(v => (
            <button key={v} onClick={() => toggle(v)}
              style={{ textAlign: "left", padding: "9px 13px", borderRadius: 10, border: `1.5px solid ${selected.includes(v) ? C.coral : "rgba(255,255,255,0.12)"}`, background: selected.includes(v) ? "rgba(232,93,74,0.2)" : "rgba(255,255,255,0.04)", color: selected.includes(v) ? "#ff9980" : "#ccc", cursor: "pointer", fontFamily: "Nunito", fontSize: 13, fontWeight: selected.includes(v) ? 700 : 400 }}>
              {selected.includes(v) ? "✓ " : ""}{v}
            </button>
          ))}
        </div>
        <p style={{ fontSize: 12, color: "#aaa", marginBottom: 6, fontWeight: 700 }}>Your statement (optional):</p>
        <textarea value={details} onChange={e => setDetails(e.target.value)} placeholder="Describe what happened..."
          style={{ width: "100%", padding: "10px 13px", borderRadius: 10, border: "1.5px solid rgba(255,255,255,0.12)", background: "rgba(255,255,255,0.06)", color: "#fff", fontFamily: "Nunito", fontSize: 13, outline: "none", resize: "vertical", minHeight: 70, marginBottom: 14 }} />
        <div style={{ display: "flex", gap: 8 }}>
          <Btn onClick={onClose} color="rgba(255,255,255,0.1)" style={{ flex: 1 }}>Cancel</Btn>
          <Btn onClick={submit} disabled={loading} color={`linear-gradient(135deg,${C.coral},#b91c1c)`} style={{ flex: 2 }}>
...
// ── Chat Window ────────────────────────────────────────────
function ChatWindow({ user, peer, onClose, blockedIds, onBlock }) {
  ...
  async function send() {
    ...
    await addDoc(collection(db, "chats", roomId, "messages"), {
      from: user.id, fromName: user.name, text, ts: serverTimestamp()
    });
    // Mark chat room exists
    await setDoc(doc(db, "chats", roomId), {
      participants: [user.id, peer.id],
      participantNames: { [user.id]: user.name, [peer.id]: peer.name },
  ...
  return (
    <>
      {showReport && (
        <ReportModal peer={peer} user={user} msgs={msgs} onReport={handleReport} onClose={() => setShowReport(false)} />
      )}
      <div style={{ display: "flex", flexDirection: "column", height: "100%" }}>
        <div style={{ display: "flex", alignItems: "center", gap: 10, padding: "11px 14px", borderBottom: "1px solid rgba(255,255,255,0.1)", background: "rgba(0,0,0,0.35)" }}>
          ...
          <div style={{ flex: 1 }}>
            ...
            <div style={{ fontSize: 12, color: C.sand }}>{peer.vehicle || "Passenger"}</div>
          </div>
          <div className="pu" style={{ width: 9, height: 9, borderRadius: "50%", background: "#4ade80" }} />
          {isPassenger && (
            <button onClick={() => setShowReport(true)}
              style={{ background: "rgba(232,93,74,0.2)", border: `1px solid ${C.coral}`, borderRadius: 8, padding: "5px 10px", color: C.coral, cursor: "pointer", fontFamily: "Nunito", fontSize: 12, fontWeight: 700, marginLeft: 6 }}>
              🚨 Report
            </button>
          )}
        </div>
        <div style={{ flex: 1, overflowY: "auto", padding: 14, display: "flex", flexDirection: "column", gap: 8 }}>
          {msgs.length === 0 && (
            <div style={{ textAlign: "center", padding: 30, color: "#666", fontSize: 13 }}>
              ...
              <span style={{ fontSize: 11 }}>Messages are saved in real-time ☁️</span>
            </div>
          )}
          {msgs.map((m) => {
            ...
            return (
              <div key={m.id} style={{ display: "flex", justifyContent: mine ? "flex-end" : "flex-start" }}>
                <div style={{ maxWidth: "72%", padding: "9px 13px", borderRadius: mine ? "16px 16px 3px 16px" : "16px 16px 16px 3px", background: mine ? `linear-gradient(135deg,${C.sunset},${C.coral})` : "rgba(255,255,255,0.1)", fontSize: 14, lineHeight: 1.5 }}>
                  {m.text}
                </div>
              </div>
            );
        ...
        <div style={{ padding: 10, display: "flex", gap: 8, borderTop: "1px solid rgba(255,255,255,0.1)" }}>
          <input value={inp} onChange={e => setInp(e.target.value)} onKeyDown={e => e.key === "Enter" && send()}
            placeholder="Type a message..." style={{ flex: 1, padding: "10px 15px", borderRadius: 22, border: "none", background: "rgba(255,255,255,0.1)", color: "#fff", fontFamily: "Nunito", fontSize: 14, outline: "none" }} />
...
function MainApp({ user, onLogout }) {
  ...
  // Real-time list of online users (opposite role)
  useEffect(() => {
    ...
    const unsub = onSnapshot(q, snap => {
      const list = snap.docs
        ...
        .filter(u => u.role === targetRole && u.id !== user.id && !u.banned && !(blockedIds.includes(u.id)));
      setPeers(list);
    });
    return unsub;
  }, [blockedIds]);
  ...
  if (user.banned) {
    return (
      <div style={{ height: "100vh", display: "flex", alignItems: "center", justifyContent: "center", position: "relative", zIndex: 1, padding: 20 }}>
        <Card>
    ...
    <div style={{ height: "100vh", display: "flex", flexDirection: "column", position: "relative", zIndex: 1 }}>
      <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", padding: "11px 15px", background: "rgba(13,27,42,0.93)", backdropFilter: "blur(12px)", borderBottom: "1px solid rgba(255,255,255,0.1)" }}>
        ...
        <div style={{ display: "flex", alignItems: "center", gap: 9 }}>
          <span style={{ fontSize: 13, color: "#bbb" }}>Hi, {user.name.split(" ")[0]}! 👋</span>
          {isDriver && <div style={{ fontSize: 11, background: `${C.palm}33`, border: `1px solid ${C.palm}`, padding: "3px 9px", borderRadius: 16, color: "#4ade80" }}>✓ Trial Active</div>}
          <Btn onClick={onLogout} style={{ padding: "6px 13px", fontSize: 12 }}>Logout</Btn>
        </div>
      </div>
      <div style={{ flex: 1, overflow: "hidden" }}>
        {chat ? (
          <ChatWindow user={user} peer={chat} onClose={() => setChat(null)} blockedIds={blockedIds} onBlock={blockUser} />
        ) : (
          <div style={{ height: "100%", overflowY: "auto", padding: 15 }}>
            <div style={{ maxWidth: 500, margin: "0 auto" }}>
              ...
              <p style={{ fontSize: 12, color: "#999", marginBottom: 12 }}>{isDriver ? "Tap a passenger to offer your service" : "Tap a driver to start chatting"}</p>
              {peers.length === 0 && (
                <Card>
                  <div style={{ textAlign: "center", padding: 20 }}>
                    <div style={{ fontSize: 52, marginBottom: 10 }}>🌊</div>
                    <p style={{ fontWeight: 800, color: C.sand, fontSize: 16 }}>{isDriver ? "No passengers online yet" : "No drivers online yet"}</p>
                    <p style={{ fontSize: 13, color: "#888", marginTop: 6 }}>Share UberLand with your friends in Siargao! 🌴</p>
                  </div>
                </Card>
              )}
              <div style={{ display: "flex", flexDirection: "column", gap: 9 }}>
                {peers.map(p => (
                  <div key={p.id} onClick={() => setChat(p)} className="fi"
                    style={{ background: "rgba(255,255,255,0.06)", borderRadius: 14, padding: 13, border: "1px solid rgba(255,255,255,0.1)", cursor: "pointer", display: "flex", alignItems: "center", gap: 12, transition: "background 0.2s" }}
              ...
              <Card style={{ marginTop: 14, borderColor: `${C.ocean}44` }}>
                <p style={{ fontSize: 13, color: "#ccc", lineHeight: 1.9 }}>
                  ...
                  {isDriver ? "1. Browse passengers • 2. Tap to offer a ride • 3. Agree on pickup & fare • 4. Earn! 💰" : "1. Browse drivers • 2. Tap to message • 3. Share your pickup & destination • 4. Choose your driver! 🏝️"}
                </p>
              </Card>
...
function Auth({ onLogin }) {
  ...
  const [vtype, setVtype] = useState("Habal-habal");
    ...
    try {
      ...
      if (isLogin) {
        ...
        const userData = {
          ...
          avatar: role === "driver" ? (vtype === "Tricycle" ? "🛺" : "🏍️") : "🧑",
      ...
      <div style={{ minHeight: "100vh", display: "flex", alignItems: "center", justifyContent: "center", padding: 20, position: "relative", zIndex: 1 }}>
        <Card style={{ width: "100%", maxWidth: 360 }}>
          <div style={{ textAlign: "center", marginBottom: 18 }}>
            ...
            <Logo />
            <p style={{ color: C.sand, fontWeight: 800, marginTop: 8 }}>Admin Access</p>
          </div>
          <div style={{ display: "flex", flexDirection: "column", gap: 10 }}>
            <Inp ph="Admin password" val={adminPw} set={setAdminPw} type="password" />
            <Btn onClick={() => { if (adminPw === ADMIN_PASS) onLogin({ role: "admin", id: "admin", name: "Admin" }); else alert("Wrong password!"); }} style={{ width: "100%" }}>
              Enter Dashboard →
            </Btn>
    ...
    <div style={{ minHeight: "100vh", display: "flex", alignItems: "center", justifyContent: "center", padding: 20, position: "relative", zIndex: 1 }}>
      <Card style={{ width: "100%", maxWidth: 420 }}>
        <div style={{ textAlign: "center", marginBottom: 22 }}>
          <Logo />
          <p style={{ color: `${C.sand}88`, marginTop: 5, fontSize: 13 }}>Siargao's Island Ride Network 🌊</p>
        </div>
        {!role ? (
          <>
            ...
            <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 10, marginBottom: 12 }}>
              {[{ r: "passenger", icon: "🧑", label: "Passenger", sub: "Free forever" }, { r: "driver", icon: "🏍️", label: "Driver", sub: "1 month free trial" }].map(({ r, icon, label, sub }) => (
                <button key={r} onClick={() => setRole(r)}
                  style={{ padding: "18px 10px", borderRadius: 14, border: `2px solid ${C.ocean}`, background: "rgba(10,126,164,0.15)", cursor: "pointer", color: "#fff", fontFamily: "Nunito" }}>
            ...
            <button onClick={() => setAdminMode(true)} style={{ width: "100%", background: "rgba(255,255,255,0.05)", border: "1px solid rgba(255,255,255,0.1)", borderRadius: 10, padding: 9, color: "#777", cursor: "pointer", fontFamily: "Nunito", fontSize: 12 }}>
              🔐 Admin Login
            </button>
          </>
        ) : (
                ...
                <>
                  ...
                  <div style={{ display: "flex", gap: 8 }}>
                    <select value={vtype} onChange={e => setVtype(e.target.value)}
                      style={{ padding: "11px 10px", borderRadius: 11, border: "1.5px solid rgba(255,255,255,0.15)", background: "rgba(255,255,255,0.07)", color: "#fff", fontFamily: "Nunito", fontSize: 13, flex: "0 0 130px" }}>
                  ...
                  <div style={{ background: "rgba(244,132,95,0.13)", borderRadius: 10, padding: 11, border: `1px solid ${C.sunset}44`, fontSize: 13, color: C.sand, lineHeight: 1.7 }}>
                    ...
                    Pay via GCash: <strong style={{ color: "#4ade80", fontSize: 15 }}>{GCASH}</strong>
                  </div>
                </>
              )}
              <Btn onClick={handleAuth} disabled={loading} style={{ width: "100%" }}>
                {loading ? <Spinner /> : isLogin ? "Log In 🌴" : "Create Account 🌴"}
              </Btn>
              <button onClick={() => setIsLogin(l => !l)} style={{ background: "none", border: "none", color: C.sand, cursor: "pointer", fontFamily: "Nunito", fontSize: 13 }}>
...
function AdminDash({ onBack }) {
  ...
  function TabBtn({ id, label }) {
    return (
      <button onClick={() => setTab(id)}
        style={{ flex: 1, padding: "9px 4px", background: tab === id ? `linear-gradient(135deg,${C.sunset},${C.coral})` : "rgba(255,255,255,0.07)", border: "none", borderRadius: 10, color: "#fff", fontFamily: "Nunito", fontWeight: 800, fontSize: 11, cursor: "pointer" }}>
        {label}
        {id === "reports" && pending.length > 0 && (
          <span style={{ marginLeft: 4, background: "#e85d4a", borderRadius: "50%", padding: "1px 5px", fontSize: 10 }}>{pending.length}</span>
        )}
      </button>
    );
  }
  ...
  if (loading) return (
    <div style={{ height: "100vh", display: "flex", alignItems: "center", justifyContent: "center", position: "relative", zIndex: 1 }}>
      <div style={{ textAlign: "center" }}><Spinner /><p style={{ marginTop: 12, color: "#aaa" }}>Loading dashboard...</p></div>
    </div>
  );

  if (reviewReport) {
    return (
      <div style={{ minHeight: "100vh", position: "relative", zIndex: 1, padding: 16, overflowY: "auto" }}>
        <div style={{ maxWidth: 600, margin: "0 auto" }}>
          ...
          <Card style={{ marginBottom: 16 }}>
            ...
            {(reviewReport.violations || []).map(v => (
              <div key={v} style={{ background: "rgba(232,93,74,0.15)", border: "1px solid #e85d4a55", borderRadius: 8, padding: "6px 12px", marginBottom: 6, fontSize: 13, color: "#ff9980" }}>🚫 {v}</div>
            ))}
            {reviewReport.details && (
              <>
                <p style={{ fontWeight: 700, marginBottom: 6, fontSize: 13, marginTop: 10 }}>Passenger's Statement:</p>
                <div style={{ background: "rgba(255,255,255,0.05)", borderRadius: 8, padding: 12, fontSize: 13, color: "#ccc", lineHeight: 1.7 }}>{reviewReport.details}</div>
              </>
            )}
            <p style={{ fontWeight: 700, marginTop: 14, marginBottom: 8, fontSize: 13 }}>💬 Chat Log (Evidence):</p>
            <div style={{ background: "rgba(0,0,0,0.3)", borderRadius: 10, padding: 12, maxHeight: 200, overflowY: "auto", display: "flex", flexDirection: "column", gap: 6 }}>
              {(reviewReport.chatLog || []).map((m, i) => {
                ...
                return (
                  <div key={i} style={{ display: "flex", justifyContent: isReported ? "flex-start" : "flex-end" }}>
              ...
              {(!reviewReport.chatLog || reviewReport.chatLog.length === 0) && <p style={{ color: "#666", fontSize: 12, textAlign: "center" }}>No chat log available</p>}
            </div>
    ...
    <div style={{ minHeight: "100vh", position: "relative", zIndex: 1, padding: 16, overflowY: "auto" }}>
      <div style={{ maxWidth: 600, margin: "0 auto" }}>
        <div style={{ display: "flex", alignItems: "center", gap: 12, marginBottom: 16, paddingTop: 8 }}>
          <Logo sm />
          <span style={{ color: C.sand, fontWeight: 800 }}>Admin Dashboard</span>
          <Btn onClick={onBack} style={{ marginLeft: "auto", padding: "6px 13px", fontSize: 12 }}>← Logout</Btn>
        </div>
        <div style={{ display: "flex", gap: 6, marginBottom: 14 }}>
          <TabBtn id="overview" label="📊 Overview" />
          <TabBtn id="drivers" label="🏍️ Drivers" />
          <TabBtn id="passengers" label="🧑 Passengers" />
          ...
          <>
              ...
              {[{ icon: "🏍️", label: "Drivers", val: drivers.length, col: C.sunset }, { icon: "🧑", label: "Passengers", val: passengers.length, col: C.ocean }, { icon: "💰", label: "Subscriptions", val: `₱${drivers.filter(d => d.subscribed).length * SUB_PRICE}`, col: "#4ade80" }, { icon: "🚨", label: "Reports", val: reports.length, col: C.coral }].map(s => (
                <Card key={s.label} style={{ padding: 16 }}>
                  <div style={{ textAlign: "center" }}>
                    ...
                    <div style={{ fontWeight: 800, fontSize: 22, color: s.col, marginTop: 4 }}>{s.val}</div>
            ...
            <Card style={{ borderColor: `${C.sunset}55`, marginBottom: 12 }}>
              ...
              <p style={{ fontSize: 14, color: "#ccc", lineHeight: 1.9 }}>
                ...
                <span style={{ fontSize: 12, color: "#888" }}>Confirm payment, then update driver status manually.</span>
              </p>
            </Card>
            {banned.length > 0 && (
              <Card style={{ borderColor: `${C.coral}55` }}>
                <p style={{ fontSize: 13, color: "#ff9980" }}><strong style={{ color: C.coral }}>🚫 Banned accounts: {banned.length}</strong><br />These users can no longer access UberLand.</p>
              </Card>
            )}
          </>
        )}
        ...
        {tab === "drivers" && (
          <Card>
            ...
            {drivers.map(d => (
              <div key={d.id} style={{ display: "flex", alignItems: "center", gap: 10, padding: "10px 0", borderBottom: "1px solid rgba(255,255,255,0.07)" }}>
                ...
                <div style={{ flex: 1 }}>
                  ...
                  <div style={{ fontSize: 11, color: "#777" }}>{d.email} · {d.joinedDate}</div>
                </div>
                <div style={{ fontSize: 11, background: d.banned ? "rgba(255,0,0,0.2)" : `${C.palm}33`, border: `1px solid ${d.banned ? "#f00" : C.palm}`, padding: "3px 9px", borderRadius: 14, color: d.banned ? "#f44" : "#4ade80" }}>
            ...
            {passengers.map(p => (
              <div key={p.id} style={{ display: "flex", alignItems: "center", gap: 10, padding: "10px 0", borderBottom: "1px solid rgba(255,255,255,0.07)" }}>
                ...
                <div style={{ flex: 1 }}>
                  ...
                  <div style={{ fontSize: 11, color: "#777" }}>{p.email} · {p.joinedDate}</div>
                </div>
          ...
          <>
            ...
            {reports.length === 0 && (
              <Card><div style={{ textAlign: "center", padding: 20 }}>
                ...
                <p style={{ color: "#4ade80", fontWeight: 800 }}>No reports yet!</p>
                <p style={{ fontSize: 13, color: "#888", marginTop: 4 }}>The community is behaving well 🌴</p>
              </div></Card>
            )}
            <div style={{ display: "flex", flexDirection: "column", gap: 10 }}>
              {reports.map(r => (
                <Card key={r.id} style={{ borderColor: r.status === "pending" ? `${C.coral}55` : "rgba(255,255,255,0.07)" }}>
                  <div style={{ display: "flex", alignItems: "flex-start", justifyContent: "space-between", marginBottom: 8 }}>
                    <div>
                      <p style={{ fontWeight: 800, color: C.coral }}>🚨 {r.reportedUserName}</p>
                      <p style={{ fontSize: 12, color: C.sand }}>{r.reportedUserVehicle}</p>
                  ...
                  <div style={{ display: "flex", flexWrap: "wrap", gap: 5, marginBottom: 10 }}>
                    {(r.violations || []).map(v => (
                      <span key={v} style={{ fontSize: 11, background: "rgba(232,93,74,0.15)", border: "1px solid #e85d4a55", borderRadius: 6, padding: "3px 8px", color: "#ff9980" }}>🚫 {v}</span>
                    ))}
                  </div>
                  {r.status === "pending" && (
                    <Btn onClick={() => setReviewReport(r)} color={`linear-gradient(135deg,${C.ocean},${C.deep})`} style={{ width: "100%", padding: "8px" }}>
                      🔍 Review Chat &amp; Decide
                    </Btn>
                  )}
                </Card>
    ...
    <>
      ...
      <div style={{ height: "100vh", display: "flex", alignItems: "center", justifyContent: "center", position: "relative", zIndex: 1 }}>
        <div style={{ textAlign: "center" }}>
          ...
          <p style={{ marginTop: 12, color: "#aaa", fontSize: 13 }}>Loading UberLand... 🌴</p>
        </div>
      </div>
    </>
  );
